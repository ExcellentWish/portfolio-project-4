...
This is a full-stack frameworks project built using Django, Python, HTML, CSS, and JavaScript. This is a restaurant website designed to display menus to customers & allow them to make/edit/delete reservations. It is based on a real Italian restaurant that serves great food. This project has been built for educational purposes.
...
- People who like real Italian food.
- People that enjoy eating out.
  
These users will be looking for:
- An informative website, with information that is easy-to-find & concise
- Current & up-to-date menus, with essential information such as price, allergens, and dietary information
...
6. As an admin user I can create, remove, update or delete food & drink items from the database so that I can ensure items are accurate and able to be added to the menu on the website.
...
9. As a user I am prompted to register for an account so that I can create an account and receive the benefits of having a profile.
10. As a user I can log in so that I can auto-populate forms with my information on the site.
11. As a user I can view the food & drink menu's separately so that I can easily find the information I'm looking for.
...
17. As a logged-in customer I can edit/delete an existing inquiry so that I can make changes if required online.
...
- Menus page, with links to food & drink menus respectively
- Reservations page, with the booking form to enquire with the restaurant
...
There are 3 models in this app, Customer, Table & Reservation. The combination of these 3 models allows for customer details to be stored, reservation enquiries to be made & managed & also enables availability checks whilst the user is enquiring.
...
## Surface
Initially, the colour pallete I have chosen for this website is from [coolers.co](https://coolors.co/5d737e-fdf7f7-64b6ac-e85d75-c76d7e).
...
**Navigation bar**: The navigation bar has links to all the active pages for the user and is clearly labelled, the menus option has a dropdown link to take the user to the Food or Drink menu. The page that the user is on has an 'active' style, the background changes to white and the text turns green to indicate to the user which page they're on. The same style change also happens when a nav link is hovered on to again clearly indicate to the user what they are about to click on. 
...
- **Footer**: The footer displays some of the restaurants' key information and has links to social media(They do not have an Instagram or Twitter, but I thought it important for my website). It is split into three sections, 'Opening Times', 'Find us! & 'Keep in touch'.
...
- **Separate menus**: Each page displays all sections of the menus separately, each menu item has the Dish/Drink name, dish/drink description, price, dietary information & any allergens. The menus are controlled by the admin user, if 'on menu' is selected in the admin panel then the item will be displayed.

I have chosen to display the menus on different pages so that the user can find the information they want as easily as possible, this separation prevents information overload as the pages aren't full of multiple menus.
...
If the user is logged in and exists in the customer model then their name & email address are pre-populated, this has been done to improve their overall experience
...
**Manage Reservations**:Logged-in users can view the 'manage reservations' page, on this page they are shown any reservation enquiries they have previously made using the email address associated with their user account.
...
There are also edit & delete buttons, users can edit or delete existing reservations they have that are either 'confirmed' or 'pending'.

![](assets/images/MangeReservations.PNG)

This image above was taken while testing. I realised that my manage reservation pages were dull as I forgot to add styling to them.

![](assets/images/MangeReservations-afterstyle.PNG)

I decided that reservations with a date in the past would display with a status of 'expired' and would not be able to be edited as this could confuse the user and also the admin user. They can be deleted, as for a user would not like having an expired reservation left there page.
...
#### Contact Form
- **Contact form**:All users can submit a contact form from the 'Contact Us' page, this sends an email to the website owner using Google SMTP. Having a way to communicate with the website owner/restaurant manager is a helpful tool for the user and creates a platform for communication to strengthen the relationships with customers.
...
- At various stages of my testing, upon submitting the contact form some users received a 500 error, this was due to Gmail preventing my application from logging in. I would receive an email to alert me of this login attempt and so I have had to enable these permissions a handful of times for it to work.
...
After further review, it was due to the changing of databases, were I had similar usernames but different passwords
...
- https://startbootstrap.com/theme/agency helped with my header
...
- https://www.youtube.com/watch?v=llbtoQTt4qw A video sent from Kasia Bogucka and it helped with this project and help me understand Django a lot.
...
- https://davidwalsh.name/javascript-debounce-function David Walsh for script.js functions
- https://getbootstrap.com/docs/4.0/getting-started/introduction/ For a lot of code used throughout my project
